...
#   4. Candidate with requires=[PLAN-AA], AA archived as blocked → exit 1
...
# -- Scenario 4: BB requires AA, AA archived as blocked → exit 1 --
jq '.status = "blocked"' "$PLANS_DIR/PLAN-AA-base.state.json" > "$ARCHIVE_DIR/PLAN-AA-base.state.json"

(cd "$TMPROOT" && bash "$PROMOTE_SCRIPT" ".claude/plans/PLAN-BB-dep.state.json" >/dev/null 2>&1)
_rc=$?
if [ "$_rc" = "1" ]; then
  pass "scenario 4: BB waits when AA archived:blocked (exit 1)"
else
  fail "scenario 4: expected exit 1, got $_rc"
fi
